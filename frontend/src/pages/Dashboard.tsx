      ...
      try {
        ...
        try {
          ...
          if (userRes.ok) {
            ...
            if (googleFitConnected) {
              const stepsRes = await fetch(`${API_BASE_URL}/api/google-fit/steps`, {
                headers: token ? { Authorization: `Bearer ${token}` } : {}
              });
              if (stepsRes.ok) {
        ...
        try {
          const nutritionRes = await fetch(
            `${API_BASE_URL}/api/nutrition/plan/${userId}/${today}`,
            { credentials: 'include' }
          );
          if (nutritionRes.ok) {
        ...
        try {
          const medicineRes = await fetch(
            `${API_BASE_URL}/api/medicines/${userId}/today`,
            { credentials: 'include' }
          );
          if (medicineRes.ok) {
        ...
        try {
          const exerciseRes = await fetch(
            `${API_BASE_URL}/api/exercise/${userId}/today`,
            { credentials: 'include' }
          );
          if (exerciseRes.ok) {
        ...
        try {
          const progressRes = await fetch(
            `${API_BASE_URL}/api/progress/${userId}/week`,
            { credentials: 'include' }
          );
          if (progressRes.ok) {
